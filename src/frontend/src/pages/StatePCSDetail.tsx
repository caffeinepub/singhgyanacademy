...
const stateNames: Record<string, string> = {
  uppcs: 'UPPCS - Uttar Pradesh',
  bpsc: 'BPSC - Bihar',
  mppcs: 'MPPCS - Madhya Pradesh',
  rpsc: 'RPSC - Rajasthan',
  hpsc: 'HPSC - Haryana',
  ukpsc: 'UKPSC - Uttarakhand',
  jpsc: 'JPSC - Jharkhand',
  cgpsc: 'CGPSC - Chhattisgarh',
  mpsc: 'MPSC - Maharashtra',
  gpsc: 'GPSC - Gujarat',
  appsc: 'APPSC - Andhra Pradesh',
  tspsc: 'TSPSC - Telangana',
  wbpsc: 'WBPSC - West Bengal',
  tnpsc: 'TNPSC - Tamil Nadu',
  kpsc: 'KPSC - Karnataka',
  keralaPSC: 'Kerala PSC',
  punjabPSC: 'Punjab PSC',
  otherStatesPCS: 'Other State PCS',
};

export default function StatePCSDetail() {
  const { state } = useParams({ strict: false });
  const category = state as CourseCategory;
  const { data: courses = [], isLoading } = useGetCoursesByCategory(category);

  const stateName = stateNames[state || ''] || 'State PCS';
    ...
    <div className="container mx-auto px-4 py-12">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12">
          ...
          <h1 className="text-4xl font-bold mb-4">{stateName}</h1>
          <p className="text-lg text-muted-foreground">
            Complete preparation material for {stateName} examination
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <Alert>
            <AlertDescription>
              Content for {stateName} is being prepared. Please check back soon!
            </AlertDescription>
          </Alert>
        )}
      </div>
